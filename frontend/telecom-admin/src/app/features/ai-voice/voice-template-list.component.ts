...
import { VoiceTemplate, VoiceTemplateStatus } from './ai-voice.models';
import { VoiceTemplateEditDialogComponent } from './voice-template-edit-dialog.component';
import {
...
@Component({
  selector: 'app-voice-template-list',
  standalone: true,
  imports: [
  ...
  template: `
    <header class="page-header">
      <div><h1>Voice Templates</h1><p>Nepali/English text with {{ placeholderHint }} placeholders, ready for TTS.</p></div>
    ...
    <div class="filters">
      <mat-form-field appearance="outline" class="search">
        ...
        <input matInput [(ngModel)]="search" (keyup.enter)="applyFilters()" placeholder="Name or text" />
      </mat-form-field>
    ...
    @if (!loading() && rows().length === 0) {
      <div class="empty"><mat-icon>description</mat-icon><p>No voice templates match your filters.</p></div>
    }
    @if (rows().length > 0) {
      <mat-card appearance="outlined">
        <table mat-table [dataSource]="rows()">
          ...
          <ng-container matColumnDef="language">
            ...
            <td mat-cell *matCellDef="let t">{{ t.language }}</td>
          </ng-container>
          ...
          <ng-container matColumnDef="actions">
            ...
            <td mat-cell *matCellDef="let t">
              ...
              <mat-menu #menu="matMenu">
                <button mat-menu-item (click)="openPreview(t)"><mat-icon>graphic_eq</mat-icon> TTS Preview</button>
          ...
          <tr mat-row *matRowDef="let row; columns: cols" (click)="openPreview(row)" class="clickable-row"></tr>
        </table>
...
export class VoiceTemplateListComponent {
  private readonly api = inject(AiVoiceService);
  private readonly dialog = inject(MatDialog);
  private readonly notify = inject(NotificationService);
  private readonly router = inject(Router);

  readonly cols = ['name', 'language', 'variables', 'status', 'updated', 'actions'];
  readonly placeholderHint = '{{name}}, {{amount}}, ...';
  readonly rows = signal<VoiceTemplate[]>([]);
  readonly total = signal(0);
  readonly loading = signal(false);
  search = '';
  statusFilter: VoiceTemplateStatus | null = null;
  page = 1;
  size = 20;
  ...
  load(): void {
    ...
    this.api.listTemplates({ search: this.search || undefined, status: this.statusFilter, page: this.page, size: this.size }).subscribe({
      ...
      error: () => { this.loading.set(false); this.notify.error('Unable to load voice templates.'); },
    });
  }
  ...
  create(): void {
    this.dialog.open(VoiceTemplateEditDialogComponent, { autoFocus: false }).afterClosed().subscribe((t) => { if (t) this.load(); });
  }
  edit(t: VoiceTemplate): void {
    this.dialog.open(VoiceTemplateEditDialogComponent, { autoFocus: false, data: { template: t } }).afterClosed().subscribe((u) => { if (u) this.load(); });
  }
  openPreview(t: VoiceTemplate): void {
    void this.router.navigate(['/ai-voice/templates', t.id, 'preview']);
  }
  remove(t: VoiceTemplate): void {
    ...
    this.dialog.open(ConfirmDialogComponent, { data, width: '440px' }).afterClosed().subscribe((ok) => {
      if (!ok) return;
      this.api.deleteTemplate(t.id).subscribe({ next: () => { this.notify.success('Voice template deleted.'); this.load(); }, error: () => this.notify.error('Unable to delete.') });
    });
  }
}